...
def str2expr(extrfeat) -> str:  #extrfeat
    ...
    if len(extrfeat[0]) == 1:
        # print("进第一个")
        # print("extrfeat['0'][0]:",extrfeat[0][0])
        # print("extrfeat['0'][0][0]:",extrfeat[0][0][0])
        ...
        ex = 'Or('
        for i in range(len(extrfeat[0])):
            # print("extrfeat[0][i]:",extrfeat[0][i])
            # print("extrfeat[0][i][0]:",extrfeat[0][i][0])
            if len(extrfeat[0][i]) == 1:
                # ex.join(extrfeat[i][0])
                ex += extrfeat[0][i][0]
            elif len(extrfeat[0][i]) > 1:
                # for j in len(extrfeat[i]):
                ex += ('And(' + ','.join(str(x) for x in extrfeat[0][i]) + ')') #join(extrfeat['0'][i])
...
def pathOfWS(extrfeat):
    ...
    if len(extrfeat[1]) == 1:
        if len(extrfeat[1][0]) == 1:
            ...
            # for i in range(len(extrfeat[0])):
            expr.append('And(' + ', '.join(str(x) for x in extrfeat[1][0]) + ')')
        ...
        for i in range(len(extrfeat[1])):
            if len(extrfeat[1][i]) == 1:
                ...
                # for j in len(extrfeat[i]):
                expr.append('And(' + ','.join(str(x) for x in extrfeat[1][i]) + ')')  # join(extrfeat['0'][i])
    ...
    for t, f in extrfeat.items():
        ...
        if len(extrfeat[t]) == 1:
            if len(extrfeat[t][0]) == 1:
                ...
                ex.append(eval('And(' + ', '.join(str(x) for x in extrfeat[t][0]) + ')'))
            ...
            for i in range(len(extrfeat[t])):
                if len(extrfeat[t][i]) == 1:
                    ...
                    # for j in len(extrfeat[i]):
                    ex1 += 'And(' + ','.join(str(x) for x in extrfeat[t][i]) + ')' # join(extrfeat['0'][i])
...
for pathFormula in formulaPaths:
    ...
    # print("---------------------------------------------------------------------")
    print("测试pathFormula1：", pathFormula)
    pathFormula = eval(pathFormula)  # str --> z3
    print("测试pathFormula2：", pathFormula)
    pts = []
    for pt in ptsOld:
