...
from DecisionSet.Minds1 import MinDS1Rules
from DecisionSet.data import Data
from DecisionSet.check import ConsistencyCheck
from DecisionSet.two_stage import TwoStage
import random
...
def split_balance(start_idx, delim, string):
    if start_idx >= len(string):
        return []

    # Ensure start_idx is within the string boundaries
    start_idx = max(0, start_idx)

    # Get the substring from start_idx to the end of the string
    substring = string[start_idx:]

    # Initialize variables to keep track of parentheses balance
    balance = 0
    current_start = 0
    parts = []

    # Iterate through the substring to find delimiter positions
    for i, char in enumerate(substring):
        ...
        # If balance becomes negative, parentheses are unbalanced
        if balance < 0:
            raise ValueError("Unbalanced parentheses")

        # If the delimiter is found and parentheses are balanced
        if char == delim and balance == 0:
            # Add the part to the list
            parts.append(string[start_idx + current_start:start_idx + i])
            current_start = i + 1

    # If there's still an imbalance at the end, raise an error
    if balance != 0:
        raise ValueError("Unbalanced parentheses")

    # Add the last part to the list
    parts.append(string[start_idx + current_start:])
...
print("use method:", choice_method)
print("#################################################################")
print("######################### Formalization #########################")
print("#################################################################")
print("  ")
...
def enumeratePredicate(MaxSize, DTFlag):
    ...
    if DTFlag:
        ...
        for i in Game["appeal_constants"]:
            SizeOneExps.append(
                {'Expression': eval(i), 'Isnum': True, 'size': 1})
        if Game["var_num"] == 2:
            ...
            if (i['Isnum']):
                ...
                if i['Expression'] == v3:
                    ...
                    if Goal1 not in SigSet:
                        ...
                        ExpSet.append(i)
    li = 2
    print(f"DEBUG:: interResultPred {vars(interResultPred)}")
    if DTFlag == False:
    ...
    predGoal = []
    # print(f"p_vocabulary {p_vocabulary}")
                ...
                for choose2 in ItemsVar:
                    if choose2 != choose1 and choose2["size"] + choose1["size"] <= predSize:
                        ...
                        if str(tempPredicate) != 'False' and str(tempPredicate) != 'True':
                            ...
                            if len(preds) > 0:
                                preds = map(lambda x: x if isinstance(
                                    x, tuple) else [x], preds)
...
def nextSizeTerm(termMaxSize, DTFlag):  # return [actNum,paraNum,paraTerm]
    ...
    if DTFlag:
        sizeOneExps = []
        sizeOneExps.append(
            {'Expression': 0, 'Isnum': True, 'arity': 0, 'size': 1})
        sizeOneExps.append(
            {'Expression': 1, 'Isnum': True, 'arity': 0, 'size': 1})
        sizeOneExps.append(
            {'Expression': v1, 'Isnum': False, 'arity': 1, 'size': 1})
        if Game["var_num"] == 2:
            sizeOneExps.append(
                {'Expression': v2, 'Isnum': False, 'arity': 1, 'size': 1})
        elif Game["var_num"] == 3:
            sizeOneExps.append({'Expression': v2, 'Isnum': False, 'size': 1})
            sizeOneExps.append({'Expression': v3, 'Isnum': False, 'size': 1})
        for i in Game["appeal_constants"]:
...
def ForLabelCopy2(pts, preds):
    print("测试pts:", pts)
    print("测试preds:", preds)
    FeatureSet = []
    # print("list(map(str, preds)):",list(map(str, preds)))
    FeatureSet.append(list(map(str, preds)) + ['target'])
    for pt in pts:
        Frontset = []
        # 判断是否满足特征，满足取1，不满足取0
        for pred in preds:
            if ptSatPred(pt, pred) is True:
                Frontset.append(1)
            else:
                Frontset.append(0)
        # 判断点属于第几个标签
        Backset = []
        # print("terms:",terms)
        for term in terms:  # trems = [true，false]
            # print("term:",term)
            index = terms.index(term)
            # print("index:",index)
            # 如果在里面
            if pt in cover[term]:
                # true里放的是真实标签为false的点
                # if term is True:
                #     OnePoint.append(0)
                # else:
                #     OnePoint.append(1)
                Backset.append(index)
        FeatureSet.append(list(map(str, Frontset)) + list(map(str, Backset)))
        # print("Feature:",FeatureSet)
    return FeatureSet
# feature集合由0,1,index组成
...
def Stage(pts, preds):
    print("DEBUG:: running two stage")
    global featureSet
    featureSet = ForLabelCopy2(pts, preds)
    lengths = set(len(sublist) for sublist in featureSet)
    firstLength = len(featureSet[0])
    if len(lengths) != 1 or (len(lengths) == 1 and any(len(sublist) != firstLength for sublist in featureSet)):
        DSflag = True
        extra_elements = extrNumClass(featureSet, firstLength)
        featureSet = combineClass(featureSet, firstLength, extra_elements)
        while DSflag:
            print("DEBUG:: in DSFLAG")
            data = Data(featureSet)
            checker = ConsistencyCheck(data)
            if checker.status and checker.do() == False:
                checker.remove_inconsistent()
            stage_DS, expr = TwoStage(data).compute()
            print("DEBUG:: stage_DS => ")
            for xs in stage_DS.values():
                for value in xs:
                    print(value)
            if expr == {}:
                featureSet = combineClass(
                    featureSet, firstLength, extra_elements)
            else:
                DSflag = False
    else:
        data = Data(featureSet)
    ...
    if expr in all_DS:
        ...
        all_DS.append(expr)
        return stage_DS, expr


def Minds2(pts, preds):  # pts, preds
    global featureSet
    featureSet = ForLabelCopy2(pts, preds)
    # print("featureSet:", featureSet)
    # 检查featureSet的子列表长度是否都一样且等于第一个子列表的长度
    lengths = set(len(sublist) for sublist in featureSet)
    firstLength = len(featureSet[0])
    # 另外一种写法
    if len(lengths) != 1 or (len(lengths) == 1 and any(len(sublist) != firstLength for sublist in featureSet)):
        ...
        featureSet = combineClass(featureSet, firstLength, extra_elements)
        while DSflag:
            # print("feature2:", featureSet)
            data = Data(featureSet)
            # 检查一致性,如果不检查一致性的话，就会无限循环
            checker = ConsistencyCheck(data)
            if checker.status and checker.do() == False:
                checker.remove_inconsistent()
            # 检查完以后计算决策集
    ...
    if expr in all_DS:
        ...
        all_DS.append(expr)
    # print("term:", terms)
    # minds2_DS, expr = Learn_DS(featureSet).compute()
    return minds2_DS, expr


def Minds1(pts, preds):
    global featureSet
    featureSet = ForLabelCopy2(pts, preds)
    print("测试featureSet:", featureSet)
    # 检查featureSet的子列表长度是否都一样且等于第一个子列表的长度
    lengths = set(len(sublist) for sublist in featureSet)
    firstLength = len(featureSet[0])
    # 另外一种写法
    if len(lengths) != 1 or (len(lengths) == 1 and any(len(sublist) != firstLength for sublist in featureSet)):
        ...
        while DSflag:
            ...
            if checker.status and checker.do() == False:
                checker.remove_inconsistent()
            # 检查完以后计算决策集
            minds1_DS, expr = MinDS1Rules(data).compute()
            # 没有生成DS， 再次组合class
            if expr == {} and DSflag == True:
                featureSet = combineClass(
                    featureSet, firstLength, extra_elements)
    ...
    # print("term:", terms)
    # minds2_DS, expr = Learn_DS(featureSet).compute()
    return minds1_DS, expr


def extrNumClass(featureSet, firstLength):
    extra_elements = {}
    for i, sublist in enumerate(featureSet):
        if len(sublist) > firstLength:
            extra_elements[i] = sublist[firstLength - 1:]
        # elif len(sublist) == firstLength - 1:
        #     # 有些pt没有对应的action
        #     extra_elements[i] = None
        else:
            extra_elements[i] = None
...
def combineClass(featureSet, firstLength, extra_elements):
    allSelt = []
    # print("extra_elements:",extra_elements)
    for i, sublist in enumerate(featureSet):
        # currSelt = []
        if i in extra_elements and extra_elements[i] is not None:
            # 随机挑选一个
            value = random.choice(extra_elements[i])
            # currSelt.append(value)
            featureSet[i][firstLength - 1:] = value
    # if currSelt in allSelt:
    #     for i, sublist in enumerate(featureSet):
    #         currSelt = []
    #         if i in extra_elements and extra_elements[i] is not None:
    #             # 随机挑选一个
    ...
    #             featureSet[i][firstLength - 1:] = value
    # else:
    #     allSelt.append(currSelt)
    # print("feature:", featureSet)
    # 如果全都组合了一遍，将DSflag弄为False
    return featureSet
...
def str2expr(extrfeat) -> str:  # extrfeat
    # 如果extrafeat的长度 == 1：只有一个target：0
    # 如果长度 >= 1:有多个target：0 用or
    # print("extrfeat[0]:",extrfeat[0])
    if len(extrfeat[0]) == 1:
        if len(extrfeat[0][0]) == 1:
            return extrfeat[0][0][0]
        elif len(extrfeat[0][0]) > 1:
            # for i in range(len(extrfeat[0])):
            return 'And(' + ','.join(str(x) for x in extrfeat[0][0]) + ')'
    elif len(extrfeat[0]) > 1:
        ex = 'Or('
        for i in range(len(extrfeat[0])):
            if len(extrfeat[0][i]) == 1:
                # ex.join(extrfeat[i][0])
                ex += extrfeat[0][i][0]
            elif len(extrfeat[0][i]) > 1:
                # for j in len(extrfeat[i]):
                # join(extrfeat['0'][i])
                ex += ('And(' + ','.join(str(x) for x in extrfeat[0][i]) + ')')
            if i < len(extrfeat[0]) - 1:
                ex += ','
        ex += ')'
    print("测试ex：", ex)
    return ex
...
def DS2Act(extrfeat):
    #    # 获取extrfeat中key对应的term
    global featureSet
    keys = list(extrfeat.keys())
    # print("keys:", keys)
    for k in keys:
        # print("k:", k)
        # print("term[k]:", terms[k])
        term = terms[k]
        # 获取键值k对应的action
        action = actions[term[0]]
        print("打印:", str(action["action_name"]) + "(" + str(term)[4:])
        # return str(action["action_name"]) + "(" + str(term)[4:]
        print("action:", action)

# 将class 1的子列表表示成And公式，并存放在列表中


def pathOfWS(extrfeat):
    # 如果extrafeat的长度 == 1：只有一个target：0
    # 如果长度 >= 1:有多个target：0 用or
    expr = []
    if len(extrfeat[1]) == 1:
        if len(extrfeat[1][0]) == 1:
            expr.append(extrfeat[1][0][0])
        elif len(extrfeat[1][0]) > 1:
            # for i in range(len(extrfeat[0])):
            expr.append('And(' + ', '.join(str(x)
                        for x in extrfeat[1][0]) + ')')
    elif len(extrfeat[1]) > 1:
        for i in range(len(extrfeat[1])):
            if len(extrfeat[1][i]) == 1:
                # ex.join(extrfeat[i][0])
                expr.append(extrfeat[1][i][0])
            elif len(extrfeat[1][i]) > 1:
                # for j in len(extrfeat[i]):
                # join(extrfeat['0'][i])
                expr.append('And(' + ','.join(str(x)
                            for x in extrfeat[1][i]) + ')')
            # if i < len(extrfeat[1]) - 1:
            #     expr += ','
    # print("expr:",expr)
    return expr


def pathOfActDS(extrfeat):
    expr = []
    # t是key， f是公式
    for t, f in extrfeat.items():
        # print("term[t]:",terms[t])
        ex = []
        if len(extrfeat[t]) == 1:
            if len(extrfeat[t][0]) == 1:
                ex.append(eval(extrfeat[t][0][0]))
            elif len(extrfeat[t][0]) > 1:
                ex.append(eval('And(' + ', '.join(str(x)
                          for x in extrfeat[t][0]) + ')'))
        elif len(extrfeat[t]) > 1:
            ex1 = 'Or('
            for i in range(len(extrfeat[t])):
                if len(extrfeat[t][i]) == 1:
                    # ex.join(extrfeat[i][0])
                    ex1 += extrfeat[t][i][0]
                elif len(extrfeat[t][i]) > 1:
                    # for j in len(extrfeat[i]):
                    # join(extrfeat['0'][i])
                    ex1 += 'And(' + ','.join(str(x)
                                             for x in extrfeat[t][i]) + ')'
                if i < len(extrfeat[t]) - 1:
                    ex1 += ','
            ex1 += ')'
        # e = eval(ex)
            ex.append(eval(ex1))
...
# *****************************************************************************************************************************************
# # 函数：开始将数据进行标准化  点对应每一个标签
# def ForLabelCopy(pts, preds):
#     # print(pts)
#     # print(preds)
#     FeatureSet = []
#     FeatureSet.append(preds)
#     for pt in pts:
#         OnePoint = []
#         Frontset = []
#         # 判断是否满足特征，满足取1，不满足取0
...
#                 #     OnePoint.append(1)
#                 Backset.append(index)
#         OnePoint.append(Backset)
#         FeatureSet.append(OnePoint)
#     return FeatureSet
# # 函数：通过第一种方法实现
# def learn(pts, preds):
#     if pts == []:
...
#     if choice_minnode == 'n':
#         choose = False
#     # tree是正确的——0，1走向，root也是正确的——1，0走向
...
def chooseBestPred2(pts, preds):
    ...
    for pred in preds:
        ...
        if len(ptsYes) == 0 or len(ptsNo) == 0:
            continue
        gini = (len(ptsYes) / len(pts)) * Gini(ptsYes) + \
            (len(ptsNo) / len(pts)) * Gini(ptsNo)
...
sheet1.write(row, 0, os.path.basename(gameName).replace(".pddl", ""))
newwb.save(resultFile)
...
while (True):
    ...
    print("Labels set: \t", str(cover).replace('True', 'temp').replace(
        'False', 'True').replace('temp', 'False'))
    DS = {}
    # extrfeat = {}
    ...
    DTflag = True
    flagAdd = True
    while pts != [] and (DS == {} or DTflag == False):
        if termination_sign:
            print("Time out,about to exit the program")
            sheet1.write(row, 2, "time-out-1200s")
            newwb.save(resultFile)
        ...
        # DTflag- Continue enumerating predicates in the last place
        ...
        DTflag = True
        # *********************************************************************************************************
        if choice_method == "TwoStage":
            DS, extrfeat = Stage(pts, preds)
        if choice_method == "Minds2":
            # preds = [v2 >= 0, v2 > 1, v1 == v2, v1 == 0, v1 >= v2, v1 >= 0, v2 > v1, v2 >= 1, v2 == 1, v2 >= v1, v1 > 1, v1 == v1 + 1, v1 == v1 + v2, v1 == v2 + 1, v1 == v2 + v2, v1 == v2 - 1, v1 == 1, v1 == 2, v1 + 1 == v2 - 1, v1 + v2 == 1, v1 + v2 == 2, v2 + 1 == v1 - 1, v2 + 1 == v1 - v2, v2 + v2 == v1 - v2, v1 - 1 == 2, v1 >= v2 + 1, v1 >= v1 - 1, v1 >= v2 - 1, v1 >= 2, v2 >= v1 + 1, v2 >= v1 - 1, v2 >= v1 - v2, v1 + v2 >= 2, v2 + 1 >= v1 - 1, v2 + 1 >= v1 - v2, v1 - 1 >= v2 + 1, v1 - 1 >= v2 + v2, v1 + v2 > 2, v1 - 1 > v2 + 1, v1 - 1 > v2 + v2, v1%2 == 1, v2%2 == 0, v2%2 == 1, (v1 + v2)%2 == 0, (v1 + v2)%2 == 1]
            DS, extrfeat = Minds2(pts, preds)
            # print("extrfeat:", extrfeat)
            # print("extr验证", 0 in extrfeat)
            # print("extr[0]验证", not any(extrfeat[0]))
            # print("extrfeat[0]:", extrfeat[0])
        if choice_method == "Minds1":
            DS, extrfeat = Minds1(pts, preds)
        # print("====print desicion set====")
        # Pset(DS)
        # *********************************************************************************************************
    ...
    if DS != {} and 0 in extrfeat and any(extrfeat[0]):
        e = eval(r'' + str2expr(extrfeat))
    print("测试：", e)
    print("\n-------------------------- Learn decision tree -------------------------\n\t",
                ...
                if i == output[0] and parasLen == len(output) - 1:
                    if parasLen == 1:
                        ...
                        for num in range(0, parasLen):
                            con = con + \
                                str(action["action_parameter"][num]) + \
                                "!=" + str(output[num + 1]) + ","
                        con = con[:-1]
                        con = con + ")"
                        s.add(eval(con))
...
def printGroundAction(termlist):
    ...
    print("Find a ground action:" +
          action["action_name"] + "(" + str(termlist)[4:-1] + ")")


def printAction(termlist):
    action = actions[termlist[0]]
    print("Find action:" + action["action_name"] +
          "(" + str(termlist)[4:-1] + ")")
        ...
        for output in ptOutput:
            ...
            if flag_cover == False:
                ...
                if len(output) == 2:
                    ...
                    term = (output[0], enumerateTerm(
                        pt, output[1]), enumerateTerm(pt, output[2]))
                elif len(output) == 4:
                    term = (
...
def tree2WinningStrategy(DT) -> str:
    ...
    if (type(DT.val) == type(v1 == v2)):
        expr = "If(" + str(DT.val) + "," + tree2WinningStrategy(DT.left) + \
            "," + tree2WinningStrategy(DT.right) + ")"
    return expr
...
def tree2Act(DT):
    ...
    if type(DT.val) == type("term"):
        ...
        return str(action["action_name"]) + "(" + str(term)[4:]
    if (type(DT.val) == type(v1 == v2)):
        expr = "If(" + str(DT.val) + "," + tree2Act(DT.left) + \
            "," + tree2Act(DT.right) + ")"
    return expr
...
def defaultPreds(pathFormula):
    print("测试pathFormula", pathFormula)
    str1 = str(pathFormula)
    str1 = str1.replace(' ', '').replace("\n", "")
    arr1 = []
    arr = []
    print("原来的str1", str1)
    if str1.startswith('Not'):  # 必胜公式前面都会有Not
        str1 = str1[4:-1]
    if str1.startswith('And'):  # And(f1, f2, f3 ,f4)
        str1 = str1[4:-1]
        # arr = str1.split(",")
        arr = split_balance(0, ',', str1)
    elif str1.startswith('Or'):
        str1 = str1[3:-1]
        # arr = str1.split(",")
        arr = split_balance(0, ',', str1)
    else:  # f1
...
# formulaPaths = pathOfWS(deepcopy(resultDS))
# formulaPaths = [str(winning_formula).replace('\n', '').strip()]
formulaPaths = [re.sub(r',\s+', ',', str(winning_formula))]
...
print("测试：", formulaPaths)
for pathFormula in formulaPaths:
    if exitFlag:
        break

    print("\n******************** One of T-label path formula:",
          pathFormula, "**********************\n")

    # print("one T-label path formula:",pathFormula,)
    # print("---------------------------------------------------------------------")
    pathFormula = eval(pathFormula)  # str --> z3
    # pathFormula = parse_smt2_string(pathFormula) //需要特定格式"(And (Not (= v1 2)) (Not (= v1 (- 2 v2))) (Not (= v1 3)) (Not (= (+ v1 v2) 3)) (Not (= (* 2 v2) (- 2 v1))) (>= v2 (- 3 v1)))"
    pts = []
    for pt in ptsOld:
    ...
    # pred = defaultPreds(And(Not(v1 == 2),Not(v1 == v2 - 2),Not(v1 == 3),Not(v1 + v2 == 3),Not(v2 + 2 == v1 - v2),v2 + 1 >= v1 - 2))
    # print("defaultPreds2:\n", pred)
    ...
    while True:
        ...
        while pts != [] and (DS == {} or DTflag == False):
            ...
            if DTflag == False:
                maxSizePred += 1
                DS = None  # 感觉有问题
                print('Cannot solve ,need bigger atom szie:',
                      maxSizePred, " and more label")
        print(
            "\n---------------------------- Learn decision tree --------------------------")
        # print("\t" + Pset(DS) + "\n")
        ...
        print("测试DS:", DS)
        print("测试extrfeat:", extrfeat)
        isSAT = False
        if DS != {} and 0 in extrfeat and any(extrfeat[0]):
            ActPaths = pathOfActDS(extrfeat)
            print("Path:", pathFormula, ", all canditate strategy:",
                  ActPaths)  # [["", (0, 1, 1)]]
            isSAT = True
            winningStrategyTemp = []
            for path in ActPaths:
                ...
                print("Verify this path:", winningStrategyPath,
                      "execute action:", ActExe)
        ...
        if pts == []:
            ...
            break
        if isSAT == True:
            print("\nThe T-label path formula", pathFormula,
                  "can find partial winning strategy：")
            print("\t", Pset(DS))
