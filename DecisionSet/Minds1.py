...
#在论文1的基础上修改约束规则,在Minds2的6、7约束的基础上新增10、11、12、13约束
#==============================================================================
class MinDS1Rules(Learn_DS, object):
    """
    ...
    def __init__(self, data):
        ...
        """
        self.conset = {}
        super(MinDS1Rules, self).__init__(data)
    ...
    def extract_cover(self, model):
        ...
        for j in range(1, self.nof_terms + 1):
            ...
            if int(self.data.fvmap.opp[label][1]) in self.conset:
                self.conset[int(self.data.fvmap.opp[label][1])].append(rule.extr0feat())
            else:
                self.conset[int(self.data.fvmap.opp[label][1])] = [rule.extr0feat()]
